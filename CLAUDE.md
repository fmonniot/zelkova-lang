...
| Code generation | `src/compiler/javascript.rs` | `javascript::emit` turns one `CheckedModule` into the text of an ES module; nothing in the pipeline calls it and nothing is written. It emits the three `unsafe` `Js/*` facades under `std/core`; it refuses a `case`, an effectful facade signature, a facade with no companion, and a module holding a declaration the typer could not check. Of `std/core`'s other five modules it emits `Bitwise` and refuses the rest. Its module doc comment has the shape, the representations and the call rule |
