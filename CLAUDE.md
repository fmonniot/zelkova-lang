...
`prose-pass` tightens one that is already written without changing a rule it states. Both are
held to `docs/spec/conventions.md`, which is also what a session writing spec prose without
either skill is held to — *The words a chapter uses* and *The sentences a chapter does not need*
are the wording rules, and neither has a test behind it.
