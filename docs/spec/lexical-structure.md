...
Five further words are **soft keywords**. Each is a keyword in exactly one position and an
...
| `derived` | the body of an [instance declaration](type-classes.md#an-instance-may-be-derived) |
...
rather than the one before: alone it asks for the instance to be derived, and `derived = …` is
an ordinary binding.
...
instance declaration whose body it is does not parse at all
([`CLASS-2`](../tickets/class-2.md)). That block is green now and stays green: what the word
becomes is a keyword in one position it cannot currently occupy, not a name a program loses.
