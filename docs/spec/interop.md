...
**Not implemented:** the JavaScript the compiler produces builds a union value in the encoding
above, but nothing writes that output or runs it, no WebAssembly is produced, no predicate is run
and no interface is read; the predicates are what [`GEN-2`](../tickets/gen-2.md) emits.
