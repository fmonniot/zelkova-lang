...
- **Nothing runs a package's tests.** The `tests/` root is compiled with the package it
  belongs to, and no runner looks in it for a test.
...
can be produced. It is gated on a test runner, which does not exist, and on obtaining values of an
