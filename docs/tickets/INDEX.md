...
`ERR-3` through `ERR-7` are one body of work, not five loose items. The goal is that **every
phase can point its error at the source that caused it** — a caret under the offending text,
