...
**Blocked by:** [LANG-53](lang-53.md). See *What is left, and why it waits* below.
...
**What is left, and why it waits:** [`LANG-53`](lang-53.md). `Js/Basics.zel` and
...
one therefore turns both modules red, with no spelling available, until `LANG-53` seeds the
...
`LANG-53` implements is decision 3, and decision 1 settles [`BUG-26`](bug-26.md) with it.

Applying the fix on a branch to measure it: with the `do_types` change above, and with `Int`,
`Float`, `Bool`, `Char` and `String` seeded into *every* scope — wider than `LANG-53` seeds,
...
the seeded names. So `LANG-53` aside, the rest of the fix costs nothing beyond those two items.
